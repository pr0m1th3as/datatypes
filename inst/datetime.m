...
################################################################################
  ...
  methods (Access = public)
    ...
    ## Every @emph{other} zone answers for any year as well, though beyond the
    ## IANA database's own range there is no zone @emph{data} to consult --
    ...
    ## inside the database's range.  MATLAB extrapolates in the same way and
...
################################################################################
  ...
  methods (Access = public)
    ...
    ## (@var{T})} on a 2-by-2-by-2 array returns 2 and 4; asking for more pads
    ...
    function varargout = size (this, varargin)
      if (! isempty (varargin))
        ...
        if (nargout > 1 && numel (sz) != nargout)
          error (strcat ("datetime.size: nargout does not match the",
...
################################################################################
  ...
  methods (Access = public)
    ...
    function TF = isbetween (X, varargin)
      ...
      zoned = [! isempty(X.TimeZone), ! isempty(lo.TimeZone), ...
                         ! isempty(hi.TimeZone)];
    ...
    function out = cat (dim, varargin)
      ...
      if (any (zoned) && ! all (zoned))
        error (strcat ("datetime.cat: cannot concatenate a datetime array", ...
                       " that has a time zone with one that does not have", ...
                       " a", ...
                       " time zone."));
      endif
...
################################################################################
  ...
  methods (Access = private)
    ...
    function C = calDiff (A, B, comps, op)
      ...
      C = calendarDuration (zeros (sz), monthsOut, daysOut, Tdur, 'Format', ...
                            fmt);
    endfunction
...
function msg = calCompError (op)
  msg = strcat ("datetime.", op, ...
                ": COMPONENTS must be 'Years', 'Quarters',", ...
                " 'Months', 'Weeks', 'Days', or 'Time', or a string array", ...
                " or", ...
...
function fmt = dtSlashOrder (strs)
  ...
  elseif (any (f2 > 12))
    ...
    warning ('Octave:datetime:ambiguous-format', ...
             strcat ("datetime: the text was read with the format", ...
                     " 'MM/dd/yyyy', but 'dd/MM/yyyy' would read it just", ...
                     " as well; give 'InputFormat' to say which is", ...
                     " meant."));
  endif
endfunction
...
function args = dtHistArgs (args, xv, s2c, c2s, d2s, w2s, scope)
  ...
  if (! hasSpec)
    ev = dtClampEdges (dtCountBinEdges (xf, dtMethodBins (xf, lo, hi, ...
                       'auto'), ...
