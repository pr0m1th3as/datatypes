...
################################################################################
  ...
  methods (Access = public)
    ...
    ## (@var{str})} on a 2-by-2-by-2 array returns 2 and 4; asking for more pads
    ...
    function varargout = size (this, varargin)
      if (! isempty (varargin))
        ...
        if (nargout > 1 && numel (sz) != nargout)
          error (strcat ("string.size: nargout does not match the", ...
...
################################################################################
  ...
  methods (Access = public)
    ...
    ## @var{B} containing repeated elements of the input @var{A}, which must be
    ## a string vector.  @var{R} must be a @math{2*N} matrix of integers.
