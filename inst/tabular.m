...
################################################################################
  ...
  methods (Access = protected)
    ...
    ## The body behind 'pivot' on both classes.  Returns an errmsg body
    ## (empty on success) for the caller to raise under its own name.  The
    ## result is a table whatever the input was, its rows and columns being
    ## groups rather than the rows they came from.
    function [P, errmsg] = pivotResult (T, args_in)
      P = [];
      errmsg = '';
      ...
      if (! (ischar (outFmt) && isrow (outFmt) ...
        ...
        errmsg = "'OutputFormat' must be 'flat' or 'nested'.";
        return;
      endif
      ...
      if (! (ischar (rowPlace) && isrow (rowPlace) ...
        ...
        errmsg = strcat ("'RowLabelPlacement' must be 'variable'", ...
                       " or 'rownames'.");
        return;
      endif
      ...
      if (isempty (colvars) && isempty (rowvars))
        errmsg = strcat ("specify at least one of 'Columns' or", ...
                       " 'Rows'.");
        return;
      endif

      ## Resolve grouping and data variables.
      ## A class that groups by its row labels answers to their name here
      ## too, so a timetable pivots on its row times.  IXVAR reports 0 for
      ## such a key, there being no variable behind it.
      colGcols = {};
      colGnames = {};
      colIx = [];
      if (! isempty (colvars))
        [colGcols, colGnames, colIx] = keyColumns (T, colvars);
      endif
      rowGcols = {};
      rowGnames = {};
      rowIx = [];
      if (! isempty (rowvars))
        [rowGcols, rowGnames, rowIx] = keyColumns (T, rowvars);
      endif
      hasDV = ! isempty (datavar);
      if (hasDV)
        dvIx = resolveVarRef (T, datavar)(:)';
        if (! isscalar (dvIx))
          errmsg = strcat ("'DataVariable' must specify a single", ...
                         " variable.");
          return;
        endif
      ...
      if (ischar (method))
        ...
        if (! hasDV && (any (strcmp (method, reqDataMethods)) ...
                        || strcmp (method, 'none')))
          errmsg = sprintf ("Method '%s' requires a 'DataVariable'.", method);
          return;
        endif
      elseif (! is_function_handle (method))
        errmsg = strcat ("'Method' must be a method name or a", ...
                       " function handle.");
        return;
      endif
      isNone = ischar (method) && strcmp (method, 'none');
      if (isNone && incTot)
        errmsg = strcat ("'IncludeTotals' is not supported with", ...
                       " Method 'none'.");
        return;
      endif
      ...
      [rGid, nR, rLvlOf, rLevVals, ~, emsg] = ...
              tabular.pivot_dimension (rowGcols, n, incMiss, incEmpty);
      if (! isempty (emsg))
        errmsg = sprintf ("%s", emsg);
        return;
      endif
      [cGid, nC, cLvlOf, cLevVals, cMissLvls, emsg] = ...
              tabular.pivot_dimension (colGcols, n, incMiss, incEmpty);
      if (! isempty (emsg))
        errmsg = sprintf ("%s", emsg);
        return;
      endif
      ...
      for c = 1:nC
        if (isNone)
          [col, emsg] = tabular.pivot_none_column (dataVals, rGid, cGid, c, nR);
          if (! isempty (emsg))
            errmsg = sprintf ("%s", emsg);
            return;
          endif
          ...
          for r = 1:nR
            ...
            [v, emsg] = ...
                tabular.pivot_cell_value (method, hasDV, dataVals, ...
                                          rows, totalAssigned);
            if (! isempty (emsg))
              errmsg = sprintf ("%s", emsg);
              return;
      ...
      ## the method ('<method>_<datavar>', or just 'count' with no data
      ## variable); otherwise one per column group, from the grouping values (a
      ## missing group becomes '<missing_<varname>>').
      if (isempty (colIx))
        if (hasDV)
        ...
        for c = 1:nC
          ...
          for j = 1:numel (colIx)
            ...
            if (cMissLvls{j}(lvl))
              ...
              parts{j} = tabular.pivot_value_name (cLevVals{j}, lvl);
            endif
          endfor
      ...
      ## Column variables for the output: flat (the '<lvl>_<lvl>' data columns)
      ## or, for 'OutputFormat','nested' with two or more 'Columns' variables,
      ## nested tables grouped by the column hierarchy (the marginal-total
      ## column, if any, stays a flat outer variable).
      if (nested && numel (colIx) >= 2)
        [colVars, finalNames] = tabular.pivot_nest (dataCols(1:nC), cLvlOf, ...
                                            cLevVals, cMissLvls, colGnames);
        if (addTotalCol)
      ...
      ## row-label variable; a multi-variable or non-text row label falls back
      ## to row-name labelling instead.
      useRowNames = strcmp (rowPlace, 'rownames');
      if (incTot && ! useRowNames && addTotalRow)
        if (numel (rowIx) == 1)
          [lc, ok] = tabular.pivot_append_label (rowLabelCols{1}, totalLabel);
          if (ok)
            rowLabelCols{1} = lc;
          else
            useRowNames = true;
      ...
      if (useRowNames)
        if (isempty (rowIx))
          ...
          rn = tabular.pivot_row_names (rowLabelCols);
          if (addTotalRow)
            rn = [rn; {totalLabel}];
          endif
...
################################################################################
  ...
  methods (Static, Hidden)
    ...
    ## Append the 'Overall_<method>' totals label LABEL as one extra element of
    ## the row-label column COL for 'pivot' IncludeTotals, keeping its type.  OK
    ## is false when COL cannot hold the text label (a numeric, datetime, or
    ## duration row label), so the caller falls back to row-name labelling.
    ...
    ## Compute one 'pivot' cell value: apply METHOD to the data of the rows ROWS
    ## of one row-and-column group.  HASDV flags whether a data variable is
    ## given; DATAVALS holds its values.  'count' counts rows (or, with a data
    ## variable, the non-missing data values), 'percentage' is the row count as
    ## a percentage of TOTALASSIGNED, an empty cell takes the named method's
    ## empty value, and every other named method or function handle is applied
    ## through 'gs_apply_method'. Returns an errmsg body (empty on success)
    ...
    function [v, errmsg] = pivot_cell_value (method, hasDV, dataVals, rows, ...
      ...
      elseif (isempty (rows))
        ## An empty cell takes the method's empty value; a function handle is
        ## not invoked on an empty slice, the cell is left missing.
        if (ischar (method))
          v = tabular.pivot_empty_value (method);
        else
          v = NaN;
        endif
      else
    ...
    ## Return a one-row missing value MV of the same type as X for 'pivot'
    ## Method 'none' empty cells; OK is false when the type has no missing
    ## value.
    ...
    ## Build nested-table column variables for pivot 'OutputFormat','nested':
    ## group the flat data columns COLS (1-by-nC) by the column-grouping level
    ## hierarchy in LVLOF (nC-by-K level indices), producing one outer variable
    ## per level of the first column grouping variable -- each a nested table
    ## (recursively) whose variables are the next grouping variable's levels;
    ## the innermost level holds the data columns.  LEVVALS/MISSLVLS/GNAMES
    ## supply the level labels.
    function [vars, names] = pivot_nest (cols, lvlOf, levVals, missLvls, gnames)
      [vars, names] = tabular.pivot_nest_level (cols, lvlOf, 1, levVals, ...
                                                missLvls, gnames);
    endfunction

    function [vars, names] = pivot_nest_level (cols, lvlOf, p, levVals, ...
                                               missLvls, gnames)
      K = size (lvlOf, 2);
      vars = {};
      names = {};
      for L = unique (lvlOf(:,p)(:)')
        ...
        if (missLvls{p}(L))
          ...
          nm = tabular.pivot_value_name (levVals{p}, L);
        endif
        if (p == K)
          sub = cols(mask);
          ## leaf: one data column per full tuple
          vars{end+1} = sub{1};
        else
          [sv, sn] = tabular.pivot_nest_level (cols(mask), ...
                                               lvlOf(mask,:), p + 1, ...
    ...
    ## aggregating): column C of the pivot, length NR, of the same type as
    ## DATAVALS. Each cell must hold at most one value; an empty cell is filled
    ## with a missing value.  RGID and CGID are the per-row group indices.
    ## Returns an errmsg body (empty on success) emitted by the caller.
    function [col, errmsg] = pivot_none_column (dataVals, rGid, cGid, c, nR)
      ...
      for r = 1:nR
        rows = find (rGid == r & cGid == c);
        if (numel (rows) > 1)
          errmsg = strcat ("Method 'none' allows at most one value per", ...
                           " cell, but a group holds several; specify an", ...
                           " aggregating method.");
          return;
        elseif (numel (rows) == 1)
          srcRow(r) = rows;
    ...
    ## Build the cellstr RowNames for a 'pivot' with RowLabelPlacement
    ## 'rownames' from the row-label columns ROWLABELCOLS (a 1-by-K cell of
    ## typed columns), joining the per-variable displayed values of each row
    ## with '_'.
    function rn = pivot_row_names (rowLabelCols)
      ...
      for r = 1:nR
        parts = cell (1, K);
        for j = 1:K
          parts{j} = tabular.pivot_value_name (rowLabelCols{j}, r);
        endfor
    ...
    ## column variable name (or part of one).  Logical values render as 'true'
    ## or 'false', numeric values through 'num2str', and text, categorical,
    ## datetime, duration, and calendarDuration values through their displayed
    ## text.  A missing value renders as '<undefined>' so the resulting name is
    ## never empty.
    function s = pivot_value_name (col, k)
      ## A time is rendered as the whole set of levels renders it, not as the
      ## one value does on its own: midnight alone loses its time of day,
      ## which would name one column differently from its neighbours.
      if (isa (col, 'datetime') || isa (col, 'duration')
          || isa (col, 'calendarDuration'))
        c = cellstr (col);
        s = c{k};
        if (isempty (s))
          s = '<missing>';
...
function out = summaryVarLine (name, val, e)
  ...
  nr = size (val, 1);
  nc = size (val, 2);
  ## What a type says about itself joins the units and the description inside
  ## one parenthesis, in that order: 'categorical (none, 2 categories, a
  ## label)' rather than a bracket apiece.
  note = '';
  ...
  elseif (islogical (val))
    d = 'logical';
    note = sprintf ('%d true', sum (val));
  elseif (isa (val, 'categorical'))
    d = 'categorical';
    if (isordinal (val))
      d = 'ordinal categorical';
    endif
    note = sprintf ('%d categories', numel (categories (val)));
  elseif (iscellstr (val))
...
function summaryStatTable (labels, cells, cols)
  ...
  ## The field is as wide as the widest value in it and no wider: a heading
  ## wider than that overhangs rather than stretching the field.
  ...
  ## A column is as wide as the wider of its heading and its values.  The
  ## values are right-aligned in their own field and the heading is centred
  ## over it, so a heading wider than anything under it, 'NumMissing' over a
  ## column of single digits, overhangs on both sides rather than one.
  bw = zeros (1, numel (cols));
  for k = 1:numel (cols)
    bw(k) = max (cw(k), numel (cols{k}));
  endfor
  gap = 4;
  line = blanks (4 + lw);
  for k = 1:numel (cols)
    line = [line, blanks(gap), padcentre(cols{k}, bw(k))];
    gap = 8;
  endfor
  fprintf ('%s\n', deblank (line));
  for i = 1:numel (cells)
    ...
    gap = 4;
    for k = 1:numel (cols)
      field = padleft (cells{i}{k}, cw(k));
      line = [line, blanks(gap), padcentre(field, bw(k))];
      gap = 8;
    endfor
    fprintf ('%s\n', deblank (line));
  endfor
...
## Centre a field in a wider block, the extra space favouring the left where
## it cannot be split evenly.
function out = padcentre (str, w)
  pad = w - numel (str);
  if (pad <= 0)
    out = str;
    return;
  ...
  left = floor (pad / 2);
  out = [blanks(left), str, blanks(pad - left)];
endfunction
