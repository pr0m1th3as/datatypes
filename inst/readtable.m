...
## @item @qcode{'ReadVariableNames'} @tab Logical; read the first row as
## variable names (default @qcode{true}).
...
## @item @qcode{'VariableNamingRule'} @tab @qcode{'modify'} or
## @qcode{'preserve'} (default @qcode{'modify'}).
...
function tbl = readtable (filename, varargin)
  ...
  switch (lower (fileType))
    case 'text'
      ...
      'ReadRowNames', readRowNames, 'RowNamesColumn', rowNamesCol, ...
      'VariableNamesRow', varNamesRow, 'NumHeaderLines', numHeaderLines, ...
      'TextType', textType, 'VariableNamingRule', namingRule, 'Delimiter', ...
      d, 'HexType', 'text');
    case 'spreadsheet'
...
%! ## spreadsheet from the file extension.  Here it reads a comma-delimited
%! ## file, taking the first row as variable names and detecting each column's
%! ## type.
%!
...
%!             ' office:value="%g"><text:p>%g</text:p>' ...
%!             '</table:table-cell>'], x, x);
...
%! readtable ('no_such.zzz')
%!error <readtable: 'FileType' must be 'text' or 'spreadsheet'.> ...
%! readtable ('x.csv', 'FileType', 'nope')
%!error <readtable: 'Delimiter' must be a character vector or string.> ...
%! readtable ('x.csv', 'Delimiter', 42)
%!error <readtable: unsupported 'Delimiter' value 'nope'.> ...
%! readtable ('x.csv', 'Delimiter', 'nope')
