...
%!error <splitapply: FUNC must be a function handle.> ...
%! splitapply (5, [1; 2], [1; 2])
%!error <splitapply: all data variables must have the same number of rows.> ...
%! splitapply (@(x, y) sum (x), [1; 2], [1; 2; 3], [1; 2])
%!error <splitapply: G must be a numeric vector with one element per row of the data variables.> ...
%! splitapply (@(x) sum (x), [1; 2; 3], [1; 2])
%!error <splitapply: G must contain positive integers.> ...
%! splitapply (@(x) sum (x), [1; 2], [0; 1])
%!error <splitapply: G must contain every integer between 1 and the number of groups.> ...
%! splitapply (@(x) sum (x), [1; 2], [1; 3])
