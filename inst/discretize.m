...
function [BIN, EDGES] = discretize (X, arg2, varargin)
  ...
  if (! isTextScalar (side) || ! any (strcmpi (charOf (side), {'left', ...
      'right'})))
...
## Included edge
%!assert_equal (discretize ([1, 2, 3, 4, 5], [1, 3, 5], 'IncludedEdge', ...
%!              'left'), ...
%!              [1, 1, 2, 2, 2])
%!assert_equal (discretize ([1, 2, 3, 4, 5], [1, 3, 5], 'IncludedEdge', ...
%!              'right'), ...
...
%!assert_equal (size (discretize (reshape (1:24, 2, 3, 4), [1, 10, 20, ...
%!              30])), ...
%!              [2, 3, 4])
...
%!              'double')
%!assert_equal (discretize ([0, 1, 2, 3, 4, 5, 6], [1, 3, 5], int8 ([10, ...
%!              20])), ...
